...
export default function App() {
  ...
  const dsoDpoYDomain = useMemo(() => {
    if (!sensitivityData.length) return [-1000, 1000];
    let maxPct = 0;
    for (const pt of sensitivityData) {
      if (Math.abs(pt.dsoImpactPct) > maxPct) maxPct = Math.abs(pt.dsoImpactPct);
      if (Math.abs(pt.dpoImpactPct) > maxPct) maxPct = Math.abs(pt.dpoImpactPct);
    }
    const finalMax = Math.max(10, Math.ceil(maxPct * 1.1));
              ...
              <div className="lg:col-span-8 bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-6">
                  ...
                  <div className="text-left sm:text-right mt-4 sm:mt-0">
                    <span className="text-5xl font-black text-indigo-600 tracking-tighter">{Number(metrics.currentCcc).toFixed(1).replace(/\.0$/, '')}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-auto">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-end">
                      <span className="text-2xl font-bold text-slate-800">{Number(metrics.currentDio).toFixed(1).replace(/\.0$/, '')}</span>
                      {dioAdjustment !== 0 && <span className="text-xs font-medium text-indigo-500 mb-1">{dioAdjustment > 0 ? '+' : ''}{Number(dioAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>}
                    </div>
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-end">
                      <span className="text-2xl font-bold text-slate-800">{Number(metrics.currentDso).toFixed(1).replace(/\.0$/, '')}</span>
                      {dsoAdjustment !== 0 && <span className="text-xs font-medium text-indigo-500 mb-1">{dsoAdjustment > 0 ? '+' : ''}{Number(dsoAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>}
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-4 overflow-hidden">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-end">
                      <span className="text-2xl font-bold text-slate-800">{Number(metrics.currentDpo).toFixed(1).replace(/\.0$/, '')}</span>
                      {dpoAdjustment !== 0 && <span className="text-xs font-medium text-indigo-500 mb-1">{dpoAdjustment > 0 ? '+' : ''}{Number(dpoAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>}
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-4 overflow-hidden">
                ...
                <div className="space-y-6 flex-grow">
                  <div>
                    <div className="flex justify-between text-xs mb-3">
                      ...
                      <span className="font-mono bg-indigo-800 px-2 py-0.5 rounded text-indigo-200">{dioAdjustment > 0 ? '+' : ''}{Number(dioAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>
                    </div>
                    <input 
                      type="range" 
                      min="-30" max="30" step="1"
                      value={dioAdjustment}
                      onChange={(e) => setDioAdjustment(parseFloat(e.target.value))}
                  ...
                  <div>
                    <div className="flex justify-between text-xs mb-3">
                      ...
                      <span className="font-mono bg-indigo-800 px-2 py-0.5 rounded text-indigo-200">{dsoAdjustment > 0 ? '+' : ''}{Number(dsoAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>
                    </div>
                    <input 
                      type="range" 
                      min="-30" max="30" step="1"
                      value={dsoAdjustment}
                      onChange={(e) => setDsoAdjustment(parseFloat(e.target.value))}
                      className="w-full h-1.5 bg-indigo-700 rounded-lg appearance-none cursor-pointer accent-indigo-300"
                    />
                  </div>
                  <div>
                    <div className="flex justify-between text-xs mb-3">
                      ...
                      <span className="font-mono bg-indigo-800 px-2 py-0.5 rounded text-indigo-200">{dpoAdjustment > 0 ? '+' : ''}{Number(dpoAdjustment).toFixed(1).replace(/\.0$/, '')} days</span>
                    </div>
                    <input 
                      type="range" 
                      min="-30" max="30" step="1"
                      value={dpoAdjustment}
                      onChange={(e) => setDpoAdjustment(parseFloat(e.target.value))}
                      className="w-full h-1.5 bg-indigo-700 rounded-lg appearance-none cursor-pointer accent-indigo-300"
                    />
                  </div>
                </div>
